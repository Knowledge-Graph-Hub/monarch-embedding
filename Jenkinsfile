pipeline {
    ...
    environment {
        ...
        GCLOUD_VM='graph-embedding-old-man-yells-at-cloud-vm'
        ...
        stage('Run pipeline to create embedding') {
            steps {
                dir('./run_embedding') {
                    script{
                        sh 'env'
                        def EXIT_CODE=sh script:'gcloud compute ssh $GCLOUD_VM --zone $GCLOUD_ZONE --ssh-flag="-tt" --command="sudo runuser -l jtr4v -c \'python3 /home/jtr4v/monarch-embedding/run_embedding.py &> /home/jtr4v/error_file.txt\'"', returnStatus:true
                        sh 'gcloud compute scp --zone $GCLOUD_ZONE $GCLOUD_VM:/home/jtr4v/error_file.txt .'
