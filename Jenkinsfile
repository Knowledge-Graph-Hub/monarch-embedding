        ...
        stage('Run pipeline to create embedding') {
            steps {
                dir('./run_embedding') {
                    script{
                        sh 'env'
                        def EXIT_CODE=sh script:'gcloud compute ssh $GCLOUD_VM --zone $GCLOUD_ZONE --ssh-flag="-tt" --command="sudo runuser -l jtr4v -c \'python3 /home/jtr4v/run_embedding.py\'', returnStatus:true
